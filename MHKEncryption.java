...
public class MHKEncryption {
	private BigInteger q;
	private BigInteger r;
	private BigInteger cypherText;
	private BigInteger wsum;
	private SinglyLinkedList<BigInteger> w = new SinglyLinkedList<BigInteger>();
	private SinglyLinkedList<BigInteger> b = new SinglyLinkedList<BigInteger>();
	/**
	...
	public void generateB(){		
			for(int i = 0;i < w.getLength();i++){
				b.addAtEnd((w.get(i)).multiply(r).mod(q));
			}
	}
	/**
    ...
    public BigInteger Encryption(String text){
    	...
    	for(byte ch: text.getBytes()){
    		...
    		for(int i = 0;i < 8;i++){
    			if((chvalue & cursor) != 0){
    				cypherText = cypherText.add(b.get(bposition));
    			}
    			cursor >>= 1;
    		    bposition ++;
    		}
    ...
    public String Decryption(BigInteger text){
         ...
         * reverse the str
         */
        for(int i = w.getLength();i > 0;i--){
    		if((w.get(i-1)).compareTo(decryptor) < 1){
    			decryptor = decryptor.subtract(w.get(i-1));
    			ch += cursor2;
    		}
    		if(cursor2 == 128){
    			str.append((char) ch);
    ...
    public static void main(String[] args){
      ...
      MHKEncryption test = new MHKEncryption();
      test.generateQ();
      test.generateR();
      test.generateB();
   	  ...
   	  BigInteger encryptedText = test.Encryption(text);
   	  System.out.println(encryptedText);
   	  System.out.println("Original text is: " + test.Decryption(encryptedText));
     }
}
